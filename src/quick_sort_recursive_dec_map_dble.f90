...
!>
!> @details 
!> Run a decending on a vector of double precision values to sort them in the specified order.The vector of intergers map is permuted in the same way that the vector vec is permuted to be sorted. It is important that map be at least as long as vec.
!>
!> @param vec
...
!> @author Arin R. Bratt
!> @date 11/07/2014
pure subroutine quick_sort_recursive_dec_map_dble(vec,map)
  ! Arguments
  ...
  if (size(vec,1) <= max_interchange_sort_size) then
     call interchange_sort_dec_map_dble(vec,map)
  else
     ! Initilize values
     low = 1
     high = size(vec,1)
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) call quick_sort_recursive_dec_map_dble(vec(:high),map(:high))
        if (low < size(vec,1)) call quick_sort_recursive_dec_map_dble(vec(low:),map(low:))
     else
        if (low < size(vec,1)) call quick_sort_recursive_dec_map_dble(vec(low:),map(low:))
        if (high > 1) call quick_sort_recursive_dec_map_dble(vec(:high),map(:high))
     end if
  end if
end subroutine quick_sort_recursive_dec_map_dble
