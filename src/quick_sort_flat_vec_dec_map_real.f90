...
!>
!> @details
!> Sort a vector of real values using a flatt quick sort.
!> 
!> @param vec
...
!> @warning
!> Still working!
subroutine quick_sort_vec_dec_map_real(vec,map)
  ! Arguments
...
!     print *,low,high,idx
     if (high - low < max_interchange_sort_size) then
        ...
        if (low < high) call interchange_sort_vec_dec_map_real(vec(low:high),map)
        idx = idx - 1
     else
