...
!> @author Arin R. Bratt
!> @date 11/07/2014
pure subroutine quick_sort_recursive_asc_dble(vec)
  ! Arguments
  ...
  if (size(vec,1) <= max_interchange_sort_size) then
     call interchange_sort_asc_dble(vec)
  else
     ! Initilize values
     low = 1
     high = size(vec,1)
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) call quick_sort_recursive_asc_dble(vec(:high))
        if (low < size(vec,1)) call quick_sort_recursive_asc_dble(vec(low:))
     else
        if (low < size(vec,1)) call quick_sort_recursive_asc_dble(vec(low:))
        if (high > 1) call quick_sort_recursive_asc_dble(vec(:high))
     end if
  end if
end subroutine quick_sort_recursive_asc_dble
