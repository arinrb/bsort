...
!pure subroutine quick_sort_vec_asc_real(vec)
!> 
!> @warning
!> Still working!
recursive subroutine quick_sort_vec_asc_real(vec)
  ! Arguments
   ...
   !           ! mid low high
    ...
    ! call swap(vec(low - 1),vec(low))
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) then
            call quick_sort_vec_asc_real(vec(1:high))
        end if
   
        if (low < size(vec,1)) then
           call quick_sort_vec_asc_real(vec(low:))
        end if
     else
        if (low < size(vec,1)) then
           call quick_sort_vec_asc_real(vec(low:))
...
end subroutine quick_sort_vec_asc_real
