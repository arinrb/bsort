...
!>
!> @details 
!> Run an quick sort on a vector of double precision values to sort them in a specified order. The vector of intergers map is permuted in the same way that the vector vec is permuted to be sorted. It is important that map be of equal or greather length than vec.
!>
!> @param vec
...
!> @author Arin R. Bratt
!> @date 07/07/2014
!pure subroutine quick_sort_vec_map_dble(vec,map,ascending)
subroutine quick_sort_vec_map_dble(vec,map,ascending)
  ! Arguments
  ...
  ! Main
  if (ascending) then
     call quick_sort_vec_asc_map_dble(vec,map)
  else
     call quick_sort_vec_dec_map_dble(vec,map)
  end if
end subroutine quick_sort_vec_map_dble
