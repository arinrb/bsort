...
!> @author Arin R. Bratt
!> @date 11/07/2014
subroutine quick_sort_recursive_gen_map_real(vec,map,ord)
  ! Arguments
  ...
  if (size(vec,1) <= max_interchange_sort_size) then
     call interchange_sort_gen_map_real(vec,map,ord)
  else
     ! Initilize values
     low = 1
     high = size(vec,1)
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) call quick_sort_recursive_gen_map_real(vec(:high),map(:high),ord)
        if (low < size(vec,1)) call quick_sort_recursive_gen_map_real(vec(low:),map(low:),ord)
     else
        if (low < size(vec,1)) call quick_sort_recursive_gen_map_real(vec(low:),map(low:),ord)
        if (high > 1) call quick_sort_recursive_gen_map_real(vec(:high),map(:high),ord)
     end if
  end if
end subroutine quick_sort_recursive_gen_map_real
