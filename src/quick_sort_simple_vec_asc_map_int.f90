...
!pure subroutine quick_sort_vec_asc_map_int(vec,map)
!> 
!> @warning
!> Still working!
recursive subroutine quick_sort_vec_asc_map_int(vec,map)
  ! Arguments
   ...
   !           ! mid low high
    ...
    ! call swap(vec(low - 1),vec(low))
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) then
            call quick_sort_vec_asc_map_int(vec(1:high),map(1:high))
        end if
   
        if (low < size(vec,1)) then
           call quick_sort_vec_asc_map_int(vec(low:),map(low:))
        end if
     else
        if (low < size(vec,1)) then
           call quick_sort_vec_asc_map_int(vec(low:),map(low:))
...
end subroutine quick_sort_vec_asc_map_int
