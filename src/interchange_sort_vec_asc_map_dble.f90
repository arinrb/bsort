...
!>
!> @details 
!> Run an interchange sort on a vector of double precision values to sort them in an ascending order. The vector of intergers map is permuted in the same way that the vector vec is permuted to be sorted. It is important that map be of equal or greather length than vec.
!>
!> @param vec
...
!> @author Arin R. Bratt
!> @date 07/07/2014
pure subroutine interchange_sort_vec_asc_map_dble(vec,map)
  ! Arguments
...
end subroutine interchange_sort_vec_asc_map_dble
