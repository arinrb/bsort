...
!>
!> @details
!> Sort a vector of integer values using a flatt quick sort.
!> 
!> @param vec
...
!> @warning
!> Still working!
subroutine quick_sort_vec_dec_map_int(vec,map)
  ! Arguments
...
!     print *,low,high,idx
     if (high - low < max_interchange_sort_size) then
        ...
        if (low < high) call interchange_sort_vec_dec_map_int(vec(low:high),map)
        idx = idx - 1
     else
