...
!pure subroutine quick_sort_vec_dec_int(vec)
!> 
!> @warning
!> Still working!
subroutine quick_sort_vec_dec_int(vec)
  ! Arguments
...
!     print *,low,high,idx
     if (high - low < max_interchange_sort_size) then
        ...
        if (low < high) call interchange_sort_vec_dec_int(vec(low:high))
        idx = idx - 1
     else
...
end subroutine quick_sort_vec_dec_int
