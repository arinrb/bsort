...
!> @author Arin R. Bratt
!> @date 11/07/2014
pure subroutine quick_sort_flat_asc_map_real(vec,map)
  ! Arguments
     ...
     if (low < high) then
        ...
        do while(low < high)
           ...
           if (high - low < max_interchange_sort_size) then
              ...
              if (low < high) call interchange_sort_asc_map_real(vec(low:high),map(low:high))
              idx = idx - 1
           else
...
end subroutine quick_sort_flat_asc_map_real
