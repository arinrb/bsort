...
!> @author Arin R. Bratt
!> @date 11/07/2014
pure subroutine quick_sort_recursive_asc_map_int(vec,map)
  ! Arguments
  ...
  if (size(vec,1) <= max_interchange_sort_size) then
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) call quick_sort_recursive_asc_map_int(vec(:high),map(:high))
        if (low < size(vec,1)) call quick_sort_recursive_asc_map_int(vec(low:),map(low:))
     else
        if (low < size(vec,1)) call quick_sort_recursive_asc_map_int(vec(low:),map(low:))
        if (high > 1) call quick_sort_recursive_asc_map_int(vec(:high),map(:high))
     end if
  end if
end subroutine quick_sort_recursive_asc_map_int
