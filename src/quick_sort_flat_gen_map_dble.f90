...
!> @author Arin R. Bratt
!> @date 11/07/2014
subroutine quick_sort_flat_gen_map_dble(vec,map,ord)
  ! Arguments
     ...
     if (low < high) then
        ...
        do while(low < high)
           ...
           if (high - low < max_interchange_sort_size) then
              ...
              if (low < high) call interchange_sort_gen_map_dble(vec(low:high),map(low:high),ord)
              idx = idx - 1
           else
...
end subroutine quick_sort_flat_gen_map_dble
