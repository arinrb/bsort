...
!pure subroutine quick_sort_vec_asc_map_real(vec,map)
!> 
!> @warning
!> Still working!
recursive subroutine quick_sort_vec_asc_map_real(vec,map)
  ! Arguments
...
!  integer :: mid
  ...
  if (size(vec,1) <= max_interchange_sort_size) then
     ...
     call interchange_sort_vec_asc_map_real(vec,map)
  else
     ! Initilize values
     low = 1
     high = size(vec,1)
   ...
   !           ! mid low high
    ...
    ! call swap(vec(low - 1),vec(low))
     ...
     if (size(vec,1) - low > high - 1) then
        if (high > 1) then
            call quick_sort_vec_asc_map_real(vec(1:high),map(1:high))
        end if
   
        if (low < size(vec,1)) then
           call quick_sort_vec_asc_map_real(vec(low:),map(low:))
        end if
     else
        if (low < size(vec,1)) then
           call quick_sort_vec_asc_map_real(vec(low:),map(low:))
...
end subroutine quick_sort_vec_asc_map_real
