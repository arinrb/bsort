...
!>
!> @details 
!> Run a recursive three way quick sort on a vector of double precision values to sort them in a specified order. The vector of intergers map is permuted in the same way that the vector vec is permuted to be sorted. It is important that map be of equal or greather length than vec.
!>
!> @param vec
...
pure subroutine quick_sort_recursive_map_dble(vec,map,ascending)
  ! Arguments
  ...
  ! Main
  if (ascending) then
     call quick_sort_recursive_asc_map_dble(vec,map)
  else
     call quick_sort_recursive_dec_map_dble(vec,map)
  end if
end subroutine quick_sort_recursive_map_dble
