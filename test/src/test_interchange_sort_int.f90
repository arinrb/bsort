...
!> 
!> @details
!> Test the interchange_sort interface for integer values.
!> 
!> @author Arin R. Bratt
!> @date 07/07/2014
program test_interchange_sort_int
  use bsort
  use ord
  ...
  do i = 1,size(order_types,1)
     do j = 1,max_order
        ...
        if (order_types(i) .eq. 'asc') then 
           ...
           call cpu_time(start)
           do k = 1,test_count
              call interchange_sort(sorted_data(1:n,k),abs_order_int)
           end do
           call cpu_time(end)
           times(idx) = end - start
           sorted = .true.
  ...
  print *
end program test_interchange_sort_int
