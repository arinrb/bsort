!> @ingroup test
!> @brief
!> Test the quick_sort_flat interface.
!> 
!> @details
!> Test the quick_sort_flat interface for real values.
!> 
!> @author Arin R. Bratt
!> @date 15/07/2014
program test_quick_sort_flat_real
  use bsort
  use orders
  ...
  real :: data_eql(10 ** max_order,test_count)
  ...
  ! Headers
  print *
  print *,"Test a flat three way quick sort with real values."
  ...
  do i = 1,size(order_types,1)
     do j = 1,max_order
        ...
        if (order_types(i) .eq. 'asc') then 
           ...
           call cpu_time(start)
           do k = 1,test_count
              call interchange_sort(sorted_data(1:n,k),gt_real)
           end do
           call cpu_time(end)
           times(idx) = end - start
           sorted = .true.
           do k = 1,test_count
              if (.not. is_sorted(sorted_data(1:n,k),gt_real)) sorted = .false.
           end do
        end if
  ...
  print *
end program test_quick_sort_flat_real
