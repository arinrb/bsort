...
!> 
!> @details
!> Test the quick_sort interface for real values.
!> 
!> @author Arin R. Bratt
!> @date 07/07/2014
program test_quick_sort_real
  use bsort
  use ord
  ...
  do i = 1,test_count
     data_eql(:,i) = real(data(1,i))
  end do

  ! Headers
  print *
  print '(60("-"))'
  print *,"Test quick_sort with real values."
  ...
  do i = 1,size(order_types,1)
     do j = 1,max_order
        ...
        if (order_types(i) .eq. 'asc') then 
           ...
           call cpu_time(start)
           do k = 1,test_count
              call quick_sort(sorted_data(1:n,k),abs_order_real)
           end do
           call cpu_time(end)
           times(idx) = end - start
           sorted = .true.
  ...
  print '(115("-"))'
  print *
end program test_quick_sort_real
