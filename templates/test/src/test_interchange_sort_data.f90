module ord
  implicit none
contains
  function abs_order_#DATA_NAME#(x,y) result(out)
    #DATA_FULL# :: x
    #DATA_FULL# :: y
    logical :: out
    if (abs(x) > abs(y)) then
  ...
  end function abs_order_#DATA_NAME#
end module ord
...
!> 
!> @details
!> Test the interchange_sort interface for #DATA_FULL# values.
!> 
!> @author Arin R. Bratt
!> @date 07/07/2014
program test_interchange_sort_#DATA_NAME#
  use bsort
  use ord
  ...
  #DATA_FULL# :: data_asc(10 ** max_order,test_count)
  #DATA_FULL# :: data_dec(10 ** max_order,test_count)
  #DATA_FULL# :: data_rnd(10 ** max_order,test_count)
  #DATA_FULL# :: data_eql(10 ** max_order,test_count)
  #DATA_FULL# :: sorted_data(10 ** max_order,test_count)
  ...
  ! Headers
  print *
  print *,"Test interchange_sort with #DATA_FULL# values."
  ...
  do i = 1,size(order_types,1)
     do j = 1,max_order
        ...
        if (order_types(i) .eq. 'asc') then 
           ...
           call cpu_time(start)
           do k = 1,test_count
              call interchange_sort(sorted_data(1:n,k),abs_order_#DATA_NAME#)
           end do
           call cpu_time(end)
           times(idx) = end - start
           sorted = .true.
           do k = 1,test_count
              if (.not. is_sorted(sorted_data(1:n,k),abs_order_#DATA_NAME#)) sorted = .false.
           end do
        end if
  ...
  print *
end program test_interchange_sort_#DATA_NAME#
