!> @ingroup test
!> @brief
!> Test the #<sort><name># interface.
!> 
!> @details
!> Test the #<sort><name># interface for #<data><full># values.
!> 
!> @author Arin R. Bratt
!> @date 15/07/2014
program test#sort##data#
  use bsort
  use orders
  ...
  #<data><full># :: data_asc(10 ** max_order,test_count)
  #<data><full># :: data_dec(10 ** max_order,test_count)
  #<data><full># :: data_rnd(10 ** max_order,test_count)
  #<data><full># :: data_eql(10 ** max_order,test_count)
  #<data><full># :: sorted_data(10 ** max_order,test_count)
  ...
  do i = 1,size(order_types,1)
     do j = 1,max_order
        ...
        if (order_types(i) .eq. 'asc') then 
           ...
           call cpu_time(start)
           do k = 1,test_count
              call interchange_sort(sorted_data(1:n,k),gt#data#)
           end do
           call cpu_time(end)
           times(idx) = end - start
           sorted = .true.
           do k = 1,test_count
              if (.not. is_sorted(sorted_data(1:n,k),gt#data#)) sorted = .false.
           end do
        end if
