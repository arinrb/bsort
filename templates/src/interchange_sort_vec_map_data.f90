...
!>
!> @details 
!> Run an interchange sort on a vector of #DATA_FULL# values to sort them in a specified order. #IF#MAP#THEN#The vector of intergers #MAP_NAME# is permuted in the same way that the vector vec is permuted to be sorted. It is important that #MAP_VAR# be of equal or greather length than vec.#ENDIF#
!>
!> @param vec
...
!> @author Arin R. Bratt
!> @date 07/07/2014
pure subroutine interchange_sort_vec_#MAP_NAME#_#DATA_NAME#(vec#MAP_ARG#,ascending)
  ! Arguments
  ...
  ! Main
  if (ascending) then
     call interchange_sort_vec_asc_#MAP_NAME#_#DATA_NAME#(vec#MAP_ARG#)
  else
     call interchange_sort_vec_dec_#MAP_NAME#_#DATA_NAME#(vec#MAP_ARG#)
  end if
end subroutine interchange_sort_vec_#MAP_NAME#_#DATA_NAME#
